...
// 1. 替换Telegram时间选择器区域 - 改为早上+晚上两个独立开关和时间选择器
// 找到当前的 tgTimeRow 区域并替换
...
if (h.indexOf(oldTgAuto) < 0) { console.log('oldTgAuto not found!'); 
  // 尝试找部分匹配来调试
  const idx = h.indexOf('tgAuto');
  console.log('tgAuto at:', idx);
  if (idx > 0) console.log(h.substring(idx - 50, idx + 200));
  process.exit(1); 
}
h = h.replace(oldTgAuto, newTgAuto);
console.log('Telegram UI replaced with dual time selectors');

// 2. 替换 openPushSettings - 支持双时间配置
...
h = h.substring(0, oldOpen) + newOpen + h.substring(oldOpenEnd);
...
h = h.substring(0, oldSave) + newSave + h.substring(oldSaveEnd);
console.log('savePushSettings replaced');

// 4. 替换 autoPushCheck - Telegram检查两个时间点
...
console.log('autoPushCheck replaced with dual TG time support');
