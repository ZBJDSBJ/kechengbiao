...
const newBuild2 = 'function buildTomorrowSchedule(){const tmr=new Date();tmr.setDate(tmr.getDate()+1);const day=tmr.getDay();const dows=["周日","周一","周二","周三","周四","周五","周六"];let lines=[];lines.push("📚 明日课程");lines.push("📅 "+tmr.getFullYear()+"年"+(tmr.getMonth()+1)+"月"+tmr.getDate()+"日 "+dows[day]);lines.push("─────────────");const tmrKey=tmr.getFullYear()+"-"+(tmr.getMonth()+1)+"-"+tmr.getDate();if(typeof holidays2026!=="undefined"){const hol=holidays2026[tmrKey];if(hol&&hol.type==="holiday"){lines.push("");lines.push("🎉 明天放假 · "+hol.name);lines.push("好好休息！");return lines.join("\\n")}}if(day===0||day===6){lines.push("");lines.push("📅 明天周末无课程");lines.push("好好休息！");return lines.join("\\n")}let hasCourse=false;scheduleData.forEach(function(row){if(row.period==="午休"){lines.push("🍽️ "+row.period+" "+row.time);return}const sub=row.subjects[day-1];if(sub){hasCourse=true;lines.push("📖 "+row.period+" "+row.time);lines.push("   "+sub.name+(sub.teacher?" · "+sub.teacher:""))}});if(!hasCourse){lines.push("");lines.push("📅 明天无课程")}return lines.join("\\n")}';

h = h.substring(0, oldBuild2) + newBuild2 + '\n' + h.substring(oldBuild2End);
console.log('buildTomorrowSchedule replaced');

// 替换buildTodaySchedule（在buildTomorrowSchedule之前）
...
const newBuild1 = 'function buildTodaySchedule(){const now=new Date();const day=now.getDay();const dows=["周日","周一","周二","周三","周四","周五","周六"];let lines=[];lines.push("📚 今日课程");lines.push("📅 "+now.getFullYear()+"年"+(now.getMonth()+1)+"月"+now.getDate()+"日 "+dows[day]);lines.push("─────────────");const todayKey=now.getFullYear()+"-"+(now.getMonth()+1)+"-"+now.getDate();if(typeof holidays2026!=="undefined"){const hol=holidays2026[todayKey];if(hol&&hol.type==="holiday"){lines.push("");lines.push("🎉 今天放假 · "+hol.name);lines.push("好好休息！");return lines.join("\\n")}}if(day===0||day===6){lines.push("");lines.push("📅 周末无课程");lines.push("好好休息！");return lines.join("\\n")}let hasCourse=false;scheduleData.forEach(function(row){if(row.period==="午休"){lines.push("🍽️ "+row.period+" "+row.time);return}const sub=row.subjects[day-1];if(sub){hasCourse=true;lines.push("📖 "+row.period+" "+row.time);lines.push("   "+sub.name+(sub.teacher?" · "+sub.teacher:""))}});if(!hasCourse){lines.push("");lines.push("📅 今天无课程")}return lines.join("\\n")}';
